...
- The backend resolves binaries in this order: **`FFMPEG_PATH` / `FFPROBE_PATH`** in `.env` (recommended when launching from Cursor or other GUIs with a stale `PATH`), then **`PATH`**, then on Windows common **winget** install locations under `%LOCALAPPDATA%\Microsoft\WinGet\`.
...
- **Python 3.10+** is recommended. Python 3.9 triggers deprecation warnings from yt-dlp.
...
1. **Use Python 3.10+** for the backend venv, then **`pip install -U yt-dlp`** (releases after 2025.10.14 require 3.10+; consider a [nightly build](https://github.com/yt-dlp/yt-dlp/wiki/Installation) if YouTube still breaks).
